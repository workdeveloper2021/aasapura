...
function addtowishlist(id, sameelement) {
    // Get the name value
    var productid = id;
    sameelement.classList.toggle("active");
    // Send AJAX request
    $.ajaxSetup({
        headers: {
    ...
    $.ajax({
        url: "/addtowishlist", // Route to handle the request
        method: "POST",
        data: {
            product: productid,
        },
...
$(".submit-btn-add-to-cart").on("click", function (e) {
    e.preventDefault();
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.status == "success") {
                ...
                $(".submit-btn-add-to-cart").html("Add To Cart");
            } else {
                $(".submit-btn-add-to-cart").html("Add To Cart");
                Swal.fire({
                    title: "Error",
...
function checkandgocartdelete(cartid) {
    Swal.fire({
        title: "Are you sure?",
        text: "You can not recover this item in your cart",
        icon: "warning",
        ...
        if (result.isConfirmed) {
            window.location.href = "/deletecartitem/" + cartid;
        }
    });
}
...
function confirmationurlgo(e) {
    ...
    Swal.fire({
        ...
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        confirmButtonText: e.getAttribute("confimationtext"),
    }).then((result) => {
        if (result.isConfirmed) {
            window.location.href = e.getAttribute("srpath");
        }
    });
}
...
$("#submittingbutton").click(function () {
    $(this).html("Sending....");
});
...
function loadallreviews(productid) {
    ...
    $.ajax({
        url: "/getallreviews", // Route to handle the request
        method: "POST",
        data: {
            product: productid,
        },
        success: function (response) {
            $("#show_reviews").html(response);
        },
        error: function (xhr, status, error) {
            console.log(xhr);
        },
